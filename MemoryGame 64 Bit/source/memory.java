...
{
  if (mouseX > 1.5f*splitx+gap+width/30-150 && mouseX < (1.5f*splitx+gap+width/30-150) + (width - 2*splitx+gap)/7+25)
    if (mouseY > cards[0][0].card.height * 5.5f + 6*gapy-20 && mouseY < (cards[0][0].card.height * 5.5f + 6*gapy-20) + (height - cards[0][0].card.height * 4 + 6*gapy)/5)
      cheat();
...
public void draw()
{
  ...
  {
    image(back, 0, 0);
    image (cheat, 1.5f*splitx+gap+width/30-150, cards[0][0].card.height * 5.5f + 6*gapy-20, (width - 2*splitx+gap)/7+25, (height - cards[0][0].card.height * 4 + 6*gapy)/5);
    ...
    gui();
    scoreBoard(numPlayers, 1.5f*splitx+gap + width/30+ ((width - 2*splitx+gap)/5), cards[0][0].card.height * 4 + 6*gapy);
  }
  if (rules)
  {
    background(0);
...
public void gui ()
{
  ...
  image (restart, 1.5f*splitx+gap + width/30, cards[0][0].card.height * 4 + 5*gapy, (width - 2*splitx+gap)/5, (height - cards[0][0].card.height * 4 + 6*gapy)/4); //restart
  image (turn, 1.5f*splitx+gap + width/30, cards[0][0].card.height * 4.5f + 4*gapy, (width - 2*splitx+gap)/7, (height - cards[0][0].card.height * 4 + 6*gapy)/5); // turn
  image (rule, 1.5f*splitx+gap + width/30, cards[0][0].card.height * 4.85f + 4*gapy, (width - 2*splitx+gap)/5, (height - cards[0][0].card.height * 4 + 6*gapy)/5); //shuffle
  if (clicks == 0 && players[currentPlayer].hints > 0)
    image (hint, 1.5f*splitx+gap + width/30, cards[0][0].card.height * 5.15f + 6*gapy, (width - 2*splitx+gap)/7, (height - cards[0][0].card.height * 4 + 6*gapy)/5);//hint
  if ( matchNoFlip (click[0][0], click[0][1], click[1][0], click[1][1]) && showMatch)
    image (match, 1.5f*splitx+gap+width/30+15, cards[0][0].card.height * 5.5f + 6*gapy-20, (width - 2*splitx+gap)/7+25, (height - cards[0][0].card.height * 4 + 6*gapy)/5); //match
}

////////////////////////////////////////////GUICHECK////////////////////////////////////////////////GUICHECK///////////////////////////////////////GUICHECK////////////////////////GUICHECK
public void guiCheck(int x, int y)
{
  if ((x > 1.5f*splitx+gap + width/30) && (x <( 1.5f*splitx+gap + width/30) +(width - 2*splitx+gap)/5 -10 ) && !cheatOn)
  {
    if (y >  cards[0][0].card.height * 4 + 5*gapy && y <  cards[0][0].card.height * 4.5f + 4*gapy) //restart
      setup();
    if (y >  cards[0][0].card.height * 4.5f + 4*gapy && y <  cards[0][0].card.height * 4.5f + 4*gapy + (height - cards[0][0].card.height * 4 + 6*gapy)/5 && clicks > 0)//turn
      if (x < ( 1.5f*splitx+gap + width/30) +(width - 2*splitx+gap)/5 -60 && !matchNoFlip (click[0][0], click[0][1], click[1][0], click[1][1]))
      {
        turn();
        updateGame();
      }
    if (y > cards[0][0].card.height * 4.85f + 4*gapy && y < cards[0][0].card.height * 4.85f + 4*gapy+ (height - cards[0][0].card.height * 4 + 6*gapy)/5)//shuffle
      shuffle();
    if (y > cards[0][0].card.height * 5.3f + 6*gapy && y <( cards[0][0].card.height * 5.15f + 6*gapy) + ((height - cards[0][0].card.height * 4 + 6*gapy)/5) && clicks == 0 && !hints &&  players[currentPlayer].hints > 0)
    {
      if (!hints)
        hint(players[currentPlayer].maxhint);
    }
    if (y > cards[0][0].card.height * 5.5f + 6*gapy-20 && y < (cards[0][0].card.height * 5.5f + 6*gapy-20) + ( (height - cards[0][0].card.height * 4 + 6*gapy)/5) && matchNoFlip (click[0][0], click[0][1], click[1][0], click[1][1]))
    {
...
public boolean match (int r, int c, int r2, int c2)
{
...
/////////////////////////////////////////////////////////////MatchNoFlip//////////////////////////////////////////////////////////////////
public boolean matchNoFlip (int r, int c, int r2, int c2)
{
  if (r == r2 && c == c2)
    return false;
  else
...
public void scoreBoard(int numPlayers, float startx, float starty)
{
...
public void cheat ()
{
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--full-screen", "--bgcolor=#666666", "--stop-color=#cccccc", "memory" };
    if (passedArgs != null) {
